...
from ModelCode.Layers.groupedConv import GroupedConv2D
from ModelCode.Layers.imbedRGB import ImbedRGB
from ModelCode.Layers.localResponseNormalization import LocalResponseNormalization

class AlexNet(): 
    """ A implementation of the AlexNet Model, where the caller can specify what block
        of the model is desired.
    """
    def __init__(self, wd, num_outputs, l1, patch_sz, pool_rate, output=True, prog_pool = True):
        ...
        self.conv2 = GroupedConv2D(128, (5,5), name="GroupConv1", trainable=False)
        self.conv3 = layers.Conv2D(filters=384, kernel_size=(3,3), strides=(1,1), padding='same', activation="relu", name="Conv2", trainable=False)
        self.conv4 = GroupedConv2D(192, (3,3), name="GroupConv2", trainable=False)
        self.conv5 = GroupedConv2D(128, (3,3), name="GroupConv3", trainable=False)
        ...
        self.output_layer = layers.Dense(num_outputs, name="Output", trainable=True, kernel_regularizer=keras.regularizers.l1(self.l1))
    ...
    def get_model(self, layer_level):
        ...
        if(layer_level >= 1): # block 1
            ...
            model.add(layers.Input((self.patch_sz,self.patch_sz,1)))
